document.addEventListener("DOMContentLoaded", function () {
    // Load all bags products when the page loads
    loadBags();
});

// Helper function to load bags and display them in the category section
function loadBags() {
    const products = JSON.parse(localStorage.getItem("products")) || [];
    const bagsSection = document.getElementById("bags-list");
    bagsSection.innerHTML = ""; // Clear the section before reloading

    // Filter the products to display only bags
    ...
    bags.forEach(product => {
        ...
        bagsSection.appendChild(productItem);
    });
}
